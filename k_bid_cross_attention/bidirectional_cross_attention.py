...
class BidirectionalCrossAttention(layers.Layer):
    def __init__(self,
        ...
        self.context_norm = layers.LayerNormalization() if prenorm else layers.Identity()
        self.scale = keras.ops.rsqrt(ops.cast(dim_head, dtype="float32"))
        inner_dim = dim_head * heads
        self.dropout = layers.Dropout(dropout)
        self.context_dropout = layers.Dropout(dropout)
        self.to_qk = layers.Dense(inner_dim, use_bias=False, kernel_initializer="he_normal")
        self.context_to_qk = layers.Dense(inner_dim, use_bias=False, kernel_initializer="he_normal")
        self.to_v = layers.Dense(inner_dim, use_bias=False, kernel_initializer="he_normal")
        self.context_to_v = layers.Dense(inner_dim, use_bias=False, kernel_initializer="he_normal")
        self.to_out = layers.Dense(dim)
        self.context_to_out = layers.Dense(context_dim)
        self.talking_heads = layers.Conv2D(heads, 1, use_bias=False, kernel_initializer="he_normal") if talking_heads else layers.Identity()
